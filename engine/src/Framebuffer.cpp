...
namespace CE {
	...
	namespace Utils {
		...
		static GLenum FunShotFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void Framebuffer::Invalidate()
	{
		...
		{
			CE_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	{
		CE_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());		
